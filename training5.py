...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.view(batch_size, C, width, height)  # (B, C, H, W)
...
class ShoeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 返回随机图像作为占位符
            if self.transform:
                placeholder = torch.randn(3, 128, 128)  # 更新为128x128
            else:
...
class OptimizedShoeGAN:
    ...
    def _save_models(self):
        """保存模型和配置"""
        # 保存模型权重
        torch.save(self.generator.state_dict(), "generator_unified_attention.pth")
        torch.save(self.discriminator.state_dict(), "discriminator_unified_attention.pth")
        ...
        torch.save(model_config, "model_complete_attention.pth")
...
class UnifiedShoeDesigner:
    def __init__(self, model_path="model_complete_attention.pth", vocab_path="vocab.json"):
        # 加载词汇表
    ...
    def design_shoe(self, color, style, num_variations=5):
        """设计鞋子"""
        # 创建条件向量
...
def main():
    ...
    # 改进的数据增强变换 - 提高到128x128
    transform = transforms.Compose([
        transforms.Resize((128, 128)),  # 从64提高到128
        transforms.RandomCrop((128, 128)),  # 确保固定尺寸
    ...
    try:
        ...
        gan.train(dataloader, condition_dim, epochs=200, sample_interval=10)
...
def test_generation():
    ...
    if not os.path.exists("model_complete_attention.pth") or not os.path.exists("vocab.json"):
        print("请先运行训练完成模型")
        return
    ...
    try:
        ...
        # 生成图像
        os.makedirs("generated_shoes_attention", exist_ok=True)
        ...
        for idx, (color, style) in enumerate(test_combinations):
            try:
                ...
                plt.axis('off')
                plt.savefig(f"generated_shoes_attention/{color}_{style}.png",
        ...
        plt.tight_layout()
        plt.savefig("generated_shoes_attention/all_combinations.png", dpi=300, bbox_inches='tight')
        plt.close()

        print("测试完成！生成的图像保存在 generated_shoes_attention 目录中")
